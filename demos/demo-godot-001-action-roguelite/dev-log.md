...
在不新增图片资源的前提下，完成 P0 可选 UI 打磨：标题装饰、卡牌边框、HUD 木牌、按钮木牌风格强化。

### 做了什么

- 新增 `specs/spec-024-ui-03-no-asset-polish-01.md`。
- `main.gd`
  - 强化主面板、HUD、按钮、卡牌样式的填充、边框、圆角、阴影。
  - 开始界面增加无资源装饰文案。
  - 三选一按钮文本改为卡牌式：编号、标签、名称、描述、快捷键。
  - 调整面板 spacing 与最小尺寸。
- `todo.md`
  - 勾选 UI 0.3 无资源打磨项。


## 2026-06-30：P0 待办状态核对

### 本次目标

核对 P0 中已实现但未勾选的动画表现项，不改动战斗代码。

### 核对结果

- 近战敌人命中玩家后吸附问题：`issue-tracker.md` 中 `FEEL-009` 已为 Fixed，代码已有 recoil 逻辑。
- 攻击木尺表现：`issue-tracker.md` 中 `ANIM-002` 已为 Fixed，代码已有 `AttackSprite` 与 `AttackPreview` 表现。
- 主角受击表现：`issue-tracker.md` 中 `ANIM-003` 已为 Fixed，代码已有 hurt pose、受击抖动、红闪。

### 产出

- 仅更新 `todo.md`，将上述 3 项勾选。
